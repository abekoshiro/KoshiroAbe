%% ============================================================
%  SCM-PTR (Subarray Combining Passive Time Reversal)  ver11
%  ★新ARR対応：受信機が「複数距離 × 複数深度」に配置されたケース
...
for idx_sd = 1:Nsd
    ...
    %% エリア③-A：通常PTR（全ch直接合成）
    % ★送信矩形パルスに対する整合フィルタ（積分ダンプ＝幅Spsの移動平均）
    %   1サンプル抜き取りではなくパルス全体のエネルギーを集める → 雑音/ISI抑圧が向上
    mf            = ones(Sps,1) / Sps;
    ptr_mf        = conv(total_ptr_signal, mf);
    [~, peak_idx] = max(abs(conv(total_autocorr, mf)));   % 整合フィルタ後のピーク
    ptr_offset    = peak_idx + Sps - 1;                   % MFの群遅延(Sps-1)を考慮
    ptrSymbols    = ptr_mf(ptr_offset : Sps : ptr_offset + Sps*(numSymbols-1));
    ...
    weight_sum   = 0;
    for s = 1:NUM_SUB
        sub_mf    = conv(sub_ptr_signal(:,s), mf);            % ★整合フィルタ適用
        [~, pk_s] = max(abs(conv(sub_autocorr(:,s), mf)));
        off_s     = pk_s + Sps - 1;
        if off_s + Sps*(numSymbols-1) > length(sub_mf), continue; end
        subSym    = sub_mf(off_s : Sps : off_s + Sps*(numSymbols-1));
...
figure('Name', 'Constellation Diagrams', 'Position', [100 100 300*Nsd 560]);
for idx_sd = 1:Nsd
