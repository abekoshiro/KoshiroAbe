...
%% ★DFEパラメータ
% ★残留ISIが「広く平坦なpedestal」の場合、タップを増やすと過学習・誤り伝播で
%   かえって悪化する（Nf=Nb=80で実測崩壊）。DFEはk=0近傍のコンパクトなISIのみ
%   担当させ、天井(=PSR/集束品質)は受信機数・間隔で上げる方針に。
Nf     = 20;      % フィードフォワード（前方=プリカーサISI）
Nb     = 15;      % フィードバック（後方=ポストカーサISI）
lambda = 0.999;
delta  = 0.01;
Ntrain = 600;     % 目安 Ntrain≳数×(Nf+Nb)
